...
@pytest.fixture(scope="session")
def fee_payments_table():
    return settings.FEE_PAYMENTS_TABLE
...
@pytest.fixture(scope="session")
def telecallers_table():
    return settings.TELECALLERS_TABLE


@pytest.fixture(scope="session")
def document_types_table():
    return settings.DOCUMENT_TYPES_TABLE


@pytest.fixture(scope="session")
def lookup_values_table():
    return settings.LOOKUP_VALUES_TABLE


@pytest.fixture(scope="session")
def call_schedules_table():
    return settings.CALL_SCHEDULES_TABLE


@pytest.fixture(scope="session")
def campus_visits_table():
    return settings.CAMPUS_VISITS_TABLE


@pytest.fixture(scope="session")
def followups_table():
    return settings.FOLLOWUPS_TABLE
...
def verify_db_untouched(
    ...
    lookup_values_table,
    call_schedules_table,
    campus_visits_table,
    followups_table,
):
    """Runs around every single test. Whatever a test does — including its
    own try/finally cleanup of any row it created — the twelve tables must
