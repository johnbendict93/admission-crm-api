...
class TestListApplications:
    ...
    def test_pagination_total_and_has_more_are_accurate(
        ...
        id_a = (
            ...
            .insert({"applicant_id": existing_applicant, "programme": "B.Tech", "department": "Pytest Pagination Dept A"})
            .execute()
        ).data[0]["id"]
        id_b = (
            supabase.table(applications_table)
            .insert({"applicant_id": existing_applicant, "programme": "B.Tech", "department": "Pytest Pagination Dept B"})
            .execute()
        ).data[0]["id"]
        try:
            page = client.get("/applications/?limit=1&offset=0")
            ...
            full_page = client.get(f"/applications/?limit={full_limit}&offset=0")
            assert full_page.status_code == 200
            full_body = full_page.json()
            assert len(full_body["items"]) == full_body["total"]
            assert full_body["has_more"] is False
        finally:
            supabase.table(applications_table).delete().eq("id", id_a).execute()
            supabase.table(applications_table).delete().eq("id", id_b).execute()
