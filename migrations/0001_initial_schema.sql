...
--
-- This captures the schema as it existed in production BEFORE this
-- migrations system existed, via:
--   pg_dump --schema-only --no-owner --no-privileges --schema=public
-- run directly against the live production database, then hand-adapted
-- here only to strip statements that don't apply to a fresh migration run
-- (\restrict/\unrestrict are psql-only meta-commands from pg_dump's output,
-- and CREATE SCHEMA public / COMMENT ON SCHEMA public are omitted because
-- every Supabase project already has a public schema and those statements
-- error on one that already exists). No table/column/constraint/trigger/
-- policy definition below was altered from the live pg_dump output.
--
-- IMPORTANT: because both the dev and prod databases already had this
-- exact schema in place before this migrations system was introduced,
-- this migration is meant to be STAMPED (recorded as applied) rather than
-- executed, on both dev and prod:
...
-- Running it with `apply` against a database that already has this schema
-- will fail with "already exists" errors, which is expected — `stamp` is
-- the correct command for this one migration only. Every migration after
-- this one (0002 onward) is a genuinely new, not-yet-applied change and
-- should go through `apply` normally.
